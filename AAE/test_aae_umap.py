...
sess.run(iterator.initializer, feed_dict={data_ph: train_data, labels_ph: train_labels, batch_size_ph: batch_size, shufflebuffer_ph: train_data.shape[0], epochs_ph: epochs})
[org_imgs,gen_imgs] = sess.run([model.original,model.static_dec_out], feed_dict={model.batch_size:N_plot * N_plot,model.repl_batch_size: N_plot * N_plot,model.b_replay: False})
...
plt.imshow(org_img)
fname = log_path_dt+"/"+"org_imgs_AAE"+".png"
plt.savefig(fname)
...
plt.imshow(gen_img)
fname = log_path_dt+"/"+"gen_imgs_AAE"+".png"
plt.savefig(fname)
plt.close()
print("End save generated images")

# UMAP
[org_img_umap,org_label_umap,cont_z_umap,cat_z_softmax_umap, cluster_umap,cat_z_umap]=utils.AAE_Vdata1(train_data,train_labels,sess,model,batch_size,learning_rate,data_ph,labels_ph,batch_size_ph,shufflebuffer_ph,epochs_ph,
           iterator,num_classes,cat_latent_size)
latent_code = np.concatenate((cont_z_umap, cat_z_softmax_umap), axis=1)
...
fname = log_path_dt+"/"+"org_imgs_AAE_umap"+".png"
plt.savefig(fname, format="png")
plt.close()
print("End save original images of umap1")
##umap for latent code(cluster)
reducer = umap.UMAP(random_state = 42)
reducer.fit(latent_code)
embedding = reducer.transform(latent_code)
assert(np.all(embedding == reducer.embedding_))
embedding.shape
...
fname = log_path_dt+"/"+"latent_imgs_AAE_umap"+".png"
plt.savefig(fname, format="png")
plt.close()
...
fname = log_path_dt+"/"+"gen_imgs_AAE_umap"+".png"
plt.savefig(fname, format="png")
plt.close()

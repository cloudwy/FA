...
# UMAP
[org_img_umap,org_label_umap,cont_z_umap,cluster_umap,cat_z_umap]=utils.AAE_Vdata(train_data,train_labels,sess,model,batch_size,learning_rate,data_ph,labels_ph,batch_size_ph,shufflebuffer_ph,epochs_ph,
...
fname = log_path_dt+"/"+"org_imgs_AAE_umap"
...
plt.title('Latent code in clustering', fontsize=12)
fname = log_path_dt+"/"+"latent_imgs_AAE_umap"
plt.savefig(fname, format="png")
plt.close()
...
plt.gca().set_aspect('equal', 'datalim')
plt.colorbar(boundaries=np.arange(num_classes+1)-0.5).set_ticks(np.arange(num_classes))
plt.title('Latent Code with cluster label', fontsize=12)
fname = log_path_dt+"/"+"gen_imgs_AAE_umap"
plt.savefig(fname, format="png")
plt.close()
